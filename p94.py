...
class p94(kernelMachine):
    ...
    def bind( self, inexpr):
        '''Compute the value of a generic expression for a p94
        model.'''
    ...
    def _make_G( self):
        '''Create a generalization matrix for p94 model.'''
        s = self.symbols
        model = self.model
        if 'c' not in s:
            s[ 'c' ] = symbols('c')
    ...
    def _make_g( self, s1, s2):
        '''Create a single generalization factor for p94 model.''' 
        s = self.symbols
        n1 = len( s1 )
        n2 = len( s2 )
        k = len( set(s1) & set(s2) )
